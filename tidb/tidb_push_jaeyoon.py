...
# 2. 타겟 쿼리 리스트 (헤어 + 뷰티 + 일상)
TARGET_QUERIES = [
    # [Group 1] 헤어 스타일링
    "봉고데기 하는법 | 판고데기 스타일링 | 다이슨 에어랩 튜토리얼",
    "단발 셀프 스타일링 | 긴머리 웨이브 넣는법 | 남자 가일컷 드라이",
    "앞머리 자르기 | 셀프 레이어드컷 | 남자 헤어 스타일링",
    
    # [Group 2] 헤어 제품 리뷰
    "올리브영 헤어템 추천 | 내돈내산 샴푸 리뷰 | 헤어에센스 순위",
    "향기 좋은 샴푸 | 퍼퓸 샴푸 추천 | 탈모 샴푸 리뷰",
    "셀프 염색약 리뷰 | 탈색약 추천 | 남자 왁스 추천",
    
    # [Group 3] 메이크업 & 뷰티
    "데일리 메이크업 | 출근 메이크업 튜토리얼 | 자연스러운 화장법",
    "올리브영 화장품 추천 | 인생템 파데 | 쿠션 파운데이션 비교",
    "남자 메이크업 | 남자 피부 관리 | 남자 스킨케어 루틴",
    "눈썹 그리기 | 아이라인 잘 그리는법 | 립스틱 컬러 추천",
    
    # [Group 4] 스킨케어
    "피부 좋아지는 법 | 스킨케어 루틴 | 기초 화장품 순서",
    "여드름 관리 | 모공 관리법 | 피부결 개선",
    "선크림 추천 | 클렌징 오일 비교 | 토너 에센스 추천",
    "피부 타입별 스킨케어 | 수분크림 추천 | 레티놀 사용법",
    
    # [Group 5] 일상 브이로그 & 라이프스타일
    "일상 브이로그 | 직장인 브이로그 | 대학생 브이로그",
    "모닝 루틴 | 나이트 루틴 | 자기관리 루틴",
    "방 꾸미기 | 인테리어 소품 | 원룸 인테리어",
    "다이어트 브이로그 | 운동 루틴 | 건강한 식단",
    
    # [Group 6] 패션 & 코디
    "데일리룩 | 출근룩 코디 | 캐주얼 코디",
    "남자 패션 | 남자 코디 추천 | 남자 옷 하울",
    "올리브영 쇼핑 | 뷰티 하울 | 화장품 하울"
]
